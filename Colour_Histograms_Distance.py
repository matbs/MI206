...
# 1. Raw histogram distance between consecutive frames (using Bhattacharyya distance)
line_diff, = ax.plot([], [], 'r-', label='Raw Distance', alpha=0.5)

# 2. Smoothed histogram distance using moving average filter
line_smooth, = ax.plot([], [], 'b-', label='Smoothed', linewidth=2)

# 3. First derivative of smoothed signal (rate of change of histogram distance)
line_deriv, = ax.plot([], [], 'g-', label='Derivative', linewidth=2)

# Configure plot appearance
ax.set_title("Histogram Distance Over Time")
ax.set_xlabel("Frame Number")
ax.set_ylabel("Distance (Bhattacharyya)")
ax.legend()
...
y_diff_data = []         # Raw histogram distances
y_smooth_data = []       # Smoothed histogram distances
y_derivative_data = []   # Derivatives of smoothed signal

# ────────────────────────────────────────────────────────────────────────────
# INITIALIZE VIDEO CAPTURE
# ────────────────────────────────────────────────────────────────────────────
# Load video file for color histogram analysis using Bhattacharyya distance
...
def first_derivative(signal, time_interval):
    ...
    The derivative represents the rate of change of histogram distance.
...
while True:
    ...
    # ────────────────────────────────────────────────────────────────────────
    # STEP 3: COMPARE HISTOGRAMS USING BHATTACHARYYA DISTANCE
    # ────────────────────────────────────────────────────────────────────────
    if prev_hist is not None:
        # Calculate Bhattacharyya distance between current and previous histogram
        # Bhattacharyya distance is a measure of similarity between two probability distributions
        # 
        # Properties:
        # - Range: 0 to 1 (0 = identical histograms, 1 = completely different)
        # - More statistically robust than simple histogram sum difference
        # - Better for detecting real scene changes vs. minor illumination variations
        # 
        # Formula: d = sqrt(1 - sum(sqrt(hist1[i] * hist2[i])))
        diff = cv2.compareHist(hist, prev_hist, cv2.HISTCMP_BHATTACHARYYA)

        # ────────────────────────────────────────────────────────────────────
        # STEP 4: STORE AND SMOOTH DATA
        # ────────────────────────────────────────────────────────────────────
        # Append current frame index and distance to data lists
        x_data.append(index)
        y_diff_data.append(diff)

        # Apply moving average smoothing to distance signal
